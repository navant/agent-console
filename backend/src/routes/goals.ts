...
router.get('/file', (req: Request, res: Response) => {
  try {
    ...
    res.json({ path: filePath, content: getGoalContent(wsPath, filePath) });
  } catch (err) {
    res.status(500).json({ error: String(err) });
  }
});
...
router.put('/file', (req: Request, res: Response) => {
  try {
    ...
    saveGoalContent(wsPath, filePath, content);
...
router.delete('/file', (req: Request, res: Response) => {
  try {
    ...
    const filePath = req.query.path as string;
    if (!filePath) return res.status(400).json({ error: 'path is required' });
    deleteGoalFile(wsPath, filePath);
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: String(err) });
  }
...
router.post('/', (req: Request, res: Response) => {
  try {
    ...
    const file = createGoalFile(wsPath, filename, content);
...
router.post('/invoke', (req: Request, res: Response) => {
  try {
    ...
    if (!body.goalPath) return res.status(400).json({ error: 'goalPath is required' });
    const task = invokeGoalAsTask(wsPath, {
